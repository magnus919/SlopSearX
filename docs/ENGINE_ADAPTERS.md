...
| `supported_filters` | `{}` | Keys from `SUPPORTED_FILTER_KEYS` = `language`, `time_range`, `safesearch`, `pagination`, `date_from`, `date_to`. OpenAlex declares publication-date bounds and relative windows; other adapters retain the default (normalized to all-`false`). A declaration is a capability hint, never an enforcement claim. |
| `enforced_filters` | `{}` | Audited enforcement layer per filter (issue 187): `{"<filter>": "upstream" | "local"}`. `"upstream"` means the source applies the filter; `"local"` means the service post-filters this adapter's results (e.g. `time_range` via `published_date`). Absent keys mean "not enforced". OpenAlex declares `date_from`/`date_to` as `upstream` and `time_range` as `local`; undeclared filters remain `null`. Distinct from `supported_filters` — accepting a parameter never marks it enforced. |
...
| Tavily | `engines/tavily.py` | api | general, news | `ENGINE_TAVILY_API_KEY` |
...
> **Engine count (53).** This table is maintained to match the live adapter
