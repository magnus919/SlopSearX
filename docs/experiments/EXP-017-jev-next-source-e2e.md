...
- Hypothesis: after seeing the general search response, Jev selects useful follow-up families (`pypi`, `openalex`, `internetarchive`) with at least +0.20 more net evidence utility per held-out query than a deterministic keyword router, without dispatching an irrelevant family at a higher rate.
