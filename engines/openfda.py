...
def _search_expression(query: str, *, exact: bool) -> str:
    ...
    return " ".join(f"openfda.{field}{suffix}:{_quote_term(term)}" for field in _NAME_FIELDS for term in terms)
    ...
    async def search(
        ...
        try:
            ...
            async with self.http_client(timeout=timeout_ms / 1000.0) as client:
                ...
                diagnostic = (
                    f"omitted {omitted} openFDA label record(s) without a stable source identifier" if omitted else None
                )
        ...
        for position, (match_rank, _source_position, _key, item) in enumerate(ranked[:max_results], start=1):
            ...
            payload = build_payload(
                ...
                "drug_label",
                {
                    ...
                    "spl_set_id": _first_text(item.get("spl_set_id")) or _first_text(openfda.get("spl_set_id")) or None,
