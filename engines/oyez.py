...
Free, public Oyez search frontend/API. No auth required.
...
class OyezAdapter(EngineAdapter):
    """US Supreme Court case search via Oyez's public search frontend."""
        ...
        try:
            async with self.http_client(timeout=timeout_ms / 1000.0) as client:
                ...
                for case, source_score in _records(data):
                    ...
                    public_url = next(
                        (candidate for candidate in (_case_url(value) for value in href_candidates) if candidate),
                        "",
                    )
                    if not public_url:
                        continue
